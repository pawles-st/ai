 ...
 *
 * the heuristic value of a given board state is calculated as a linear combination of 4 factors and is represented as follows:
 * heuristic_value = w_0 * no_fours + w_1 * no_fours + w_2 * no_pairs + w_3 * block_pairs
 *
 * where w_i are constant weights for each factor, and they depend on whether the bot is playing as X or O (exact initialisation in the array above)
 *
 * the four factors' descriptions are as follows;
 *
 * - no_fours - the value is equal to the number of fours the player could still make in that game, if the other player never moves and the win/lose conditions are ignored
 *   in other words, for each possible four a 5x5 board, if there isn't a single square in that four labelled with the opponent's symbol, adds +1 to the final value
 *
 * - no_threes - same as above, but the loop is performed over all possible threes on a 5x5 board
 *
 * - no_pairs - counts the number of pairs that the player currently has on the board. Performs a similar loop as above, but adds a +1 only if both squares currently contain the player symbol
 *
 * - block_pairs - for each opponent pair currently on the board, counts the number of player symbols blocking - as in "in the way" of - that pair
 *   for example, the board below would set this value to 2 for the X player (Xs on tiles 2,4 and 5,4 each increase that count by 1):
 *
 *   1 2 3 4 5
 * 1 - - - X -
 * 2 X - - X -
 ...
 * 5 - - - X -
 *
 * in case of ties, the bot picks a random move from among the best ones
 */
