...
uint8_t calc_heuristic(uint64_t perm);
...
class Vertex {
	...
	Vertex(uint64_t value, uint64_t previous, uint8_t distance) :
		value(value), previous(previous), distance(distance), heuristic_value(calc_heuristic(value) + distance) {}
...
void add_move(Vertex& vertex, const uint8_t from, const uint8_t to, vertex_queue& vertex_q, const std::set<uint64_t>& visited, std::map<uint64_t, uint64_t>& parent) {
	...
	if (visited.find(new_value) == std::end(visited)) {
		...
		Vertex new_v = Vertex(new_value, curr_value, vertex.get_distance() + 1);
		vertex_q.push(new_v);
	}
}

void process(Vertex& vertex, vertex_queue& vertex_q, const std::set<uint64_t>& visited, std::map<uint64_t, uint64_t>& parent) {
	...
	if (empty_idx < BOARD_SIZE * (BOARD_SIZE - 1)) { // move up
		add_move(vertex, empty_idx, empty_idx + BOARD_SIZE, vertex_q, visited, parent);
	}
	if ((empty_idx % BOARD_SIZE) != BOARD_SIZE - 1) { // move left
		add_move(vertex, empty_idx, empty_idx + 1, vertex_q, visited, parent);
	}
...
int main() {
	...
	//uint64_t start_perm = 0xcb42a819e6537fd0;
	...
	*(std::end(random) - 1) = 0;
	...
	while (current.get_value() != goal) {
		...
		process(current, vertex_q, visited, parent);
	...
	std::cout << "found in " << std::dec << +distance << " moves\n";
